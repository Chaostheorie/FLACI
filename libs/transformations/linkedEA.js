	...
	function appendState(r, s) {
        ...
        for (var t of s.Out.values()) { // iterate outgoing transitions
            if (!keepTraps && !t.Target.NoTrap)
                return; // continue when removing traps, targets being traps are not appended
            if (!keepUnreachable && !t.Target.CanReach)
                return; // continue when removing unreachable, targets being unreachable are not appended
