...
*/
    ...
    function AAutomaton (a, noSave){
      ...
      self.generateSVG = function(){
        ...
        var url = "data:image/svg+xml;base64,"+btoa(source.replace(/ε/g,"&#0949;"));
...
*/
      ...
      self.askRemoveDEATrapStates = function(){
        if(self.automaton.allowPartial){
          var r = removeTrapStates(JSON.parse(self.toJSON()));
          if(r.result == "OK"){
...
//          r = removeChainsGrammar(r.grammar);
      ...
      self.transformEAtoRegExp = function(){
        ...
        if(r.result == "OK"){
          self.showAlert($translate.instant("AUTOEDIT.TRANSFORM.EATOREGEXPTEXT",{"REGEXP":'<span class="selectable LRPaddingSpan">'+r.regex+'</span>'}));
        }
      };
      ...
      self.parseUserAlphabet = function(){
        ...
        if(self.Type == "TM"){
          ...
          self.automaton.StackAlphabet.sort();
          ...
          if(self.Type == "DEA"){
            ...
            for(var z=0; z < self.automaton.Alphabet.length; z++){
              ...
              if(usedIn.length == 0 && !self.automaton.allowPartial){
                ...
                s.hasError = true; 
                if(verbose){ 
                  //self.showAlert($translate.instant("ERRORS.MISSINGCHARTRANSITION",{CHAR:self.automaton.Alphabet[z],STATE:s.Name}));
                  self.showConfirm($translate.instant("AUTOEDIT.CHECKASKTOCOMPLETE")).then(function(){
                    // OK clicked
                    self.completeDEA();
                  });
      ...
      self.addLabel = function(t,z){
        t.Labels.push(z);
        if(self.Type == "DEA" || self.Type == "NEA") t.Labels.sort();
        self.checkAutomaton();
        // we checked, try to create an undo step
        if(!noSave) self.makeUndoStep();
      };
      ...
      self.addFirstLabel = function(t, newTranstition){
        ...
        if((self.Type == "DEA" || self.Type == "NEA") && newTranstition){
      ...
      self.runSimulation = function(){
          ...
          function doNextNEAStep(){
            ...
            for(var i=0; i < neaMaschines.length; i++){
              ...
              if(m.step > 200) { // break possible infinity 
                if(!breakMessageShown) self.showAlert($translate.instant("AUTOEDIT.SIMULATIONTERMINATEDSTEPS")); 
        ...
        if(self.Type == "TM"){ 
          ...
          var step = 0;
          var pointerRange = [stackPointer,stackPointer];
          ...
          function doStepTM(machine,state){
            step++;
            if(step > 200) { // break possible infinity 
              self.showAlert($translate.instant("AUTOEDIT.SIMULATIONTERMINATEDSTEPS"));
              return;
            }
      ...
      self.startDrag = function(item, e, angle){
        ...
        if(self.transitionCircleMode){ 
          ...
          var dx = (Math.round((e.pageX - 320 + $('#SVGArea'+self.ID).scrollLeft())/self.automatonGraphZoom*100))-item.x;
