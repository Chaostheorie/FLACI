    ...
    function AGrammar (g,noSave){
      ...
      self.convertRegularGrammar2regex = function(){
        ...
        if(r.result == "OK"){
          ...
          if(r.result == "OK"){
            self.showAlert($translate.instant("AUTOEDIT.TRANSFORM.EATOREGEXPTEXT",
               {"REGEXP":'<span class="selectable">'+r.regex+'</span>'})); 
          }
        }
      };
...
//              mode.start.push({"regex":new RegExp("\\b"+escapeRegExp(self.terminals[i])+"\\b"),"token":"string","org":self.terminals[i]});
        ...
        }catch(e){console.log(e); self.parseError = e.message; }
      };
