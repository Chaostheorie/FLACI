  app.controller('RegExpController', function($scope, $location, $timeout, $interval, $mdDialog, $http, $mdMedia, $mdSidenav, $sce, $q, userLogin, $translate) { 
      ...
      if(re.length != 0) {
        ...
        if(r) do {
          ...
          if (m) {
            ...
            s += text.substring(p,m.index);
            p = m.index + m[0].length;
            s += '<span style="color:red;">'+m[0]+'</span>';
          }
        } while (m);
      }
      s += text != '' ? text.substring(p) : "";
      d.html(s);
    ...
    self.updateTestRegExp = function(hasValue){
      ...
      if(hasValue){
        ...
        mode = {start: [{"regex":new RegExp(self.testRegExp),"token":"comment","org":self.testRegExp},{"regex":/[\s\S]/,"token":"none","org":"[\\s\\S]"}]};
      }else{
