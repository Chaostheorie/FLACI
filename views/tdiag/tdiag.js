...
*/
      ...
      $mdDialog.show({
        ...
        controller: function ($scope, $mdDialog) {
          ...
          $scope.hide = function() {
            ...
            apiPost("createTDiagram",{"name":$scope.title,"description":$scope.description})
             .success(function(data) {
               if(data.result == "FAILED"){
    ...
    self.editDiagram = function (d,ev){
      ...
      $mdDialog.show({
        ...
        controller: function ($scope, $mdDialog) {
          ...
          $scope.hide = function() {
            apiPost("saveTDiagram",{"id":d.ID, "name":$scope.title,"description":$scope.description, JSON:d.JSON})
             .success(function(data) {
               if(data.result == "OK"){
                 self.loadFromServer();
                 $mdDialog.hide();
    ...
    self.runSelectedElements = function(diagram, elements){
      ...
      for(var i = 0; i < allElements.length; i++){
        ...
        if(allElements[i].written == "JS") if(picked.indexOf(allElements[i]) == -1) picked.push(allElements[i]);
      }
        ...
        if(e.written == "JS" && !e.preset){
          try{
            ...
            if(out.match("Jison generated parser")) 
              runLog.push($translate.instant("TDIAG.OUTPUT")+$translate.instant("TDIAG.LOGOUTPUTVCC")); else
              runLog.push($translate.instant("TDIAG.OUTPUT")+"\n"+out);
